...
namespace MTGAHelper.Lib.OutputLogParser
{
    public class DraftPicksCalculator
    {
        ...
        //CacheSingleton<IReadOnlyDictionary<string, DraftRatings>> draftRatings;
            ...
            )
        {
            if (cardPool.Any() == false)
                return Array.Empty<CardForDraftPick>();
            ...
            var isMissingRareLand = data.FirstOrDefault(i => i.Card.GetRarityEnumSplitRareLands() == RarityEnum.RareLand && i.NbMissingCollection > 0);
            ...
            {
                var bestRarity = data.Select(c => c.Card.Rarity).Min();
                var cardsMatching = data
                    .Where(i => i.Card.Rarity == bestRarity)
                    .Where(i => basicLandIdentifier.IsBasicLand(i.Card) == false);
