    ...
    public class SingletonEventsScheduleManager : IEventTypeCache
    {
        ...
        public void LoadEventsFromDisk()
        {
            if (File.Exists(path))
            {
                ...
                try
                {
                    eventCollection = JsonConvert.DeserializeObject<ICollection<GetActiveEventsV3Raw>>(fileText) ?? Array.Empty<GetActiveEventsV3Raw>();
                }
                catch (Exception)
                {
                    eventCollection = Array.Empty<GetActiveEventsV3Raw>();
                }
