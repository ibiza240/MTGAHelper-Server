...
namespace MTGAHelper.Entity.MtgaOutputLog
{
    ...
    public class LockableOutputLogResult
    {
        ...
        public InfoByDate<Dictionary<int, int>> GetLastCollection() => CollectionByDate.GetData().MaxBy(i => i.DateTime)
            ?? new InfoByDate<Dictionary<int, int>>(default(DateTime), new Dictionary<int, int>());

        public InfoByDate<ICollection<ConfigModelRankInfo>> GetLastRankInfo() => RankSyntheticByDate.GetData().MaxBy(i => i.DateTime)
        ...
        public InfoByDate<Dictionary<string, PlayerProgress>> GetLastProgress() => PlayerProgressByDate.GetData().MaxBy(i => i.DateTime)
            ?? new InfoByDate<Dictionary<string, PlayerProgress>>(default(DateTime), new Dictionary<string, PlayerProgress>());

        public InfoByDate<IList<PlayerQuest>> GetLastQuests() => PlayerQuestsByDate.GetData().MaxBy(i => i.DateTime)
        ...
        public InfoByDate<HashSet<string>> GetLastMtgaDecksFound() => MtgaDecksFoundByDate.GetData().MaxBy(i => i.DateTime)
        ...
        public DateSnapshotInfo GetForDate(DateTime dateFor)
        {
            var resultForDate = new DateSnapshotInfo
            {
                ...
                Matches = MatchesByDate.GetData().SingleOrDefault(i => i.DateTime.Date == dateFor)?.Info ?? Array.Empty<MatchResult>(),
                RankSynthetic = RankSyntheticByDate.GetData().SingleOrDefault(i => i.DateTime.Date == dateFor)?.Info ?? Array.Empty<ConfigModelRankInfo>(),
                ...
                PlayerQuests = PlayerQuestsByDate.GetData().SingleOrDefault(i => i.DateTime.Date == dateFor)?.Info ?? Array.Empty<PlayerQuest>(),
